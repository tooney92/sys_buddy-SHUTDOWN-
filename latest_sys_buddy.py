...
# time_dict2.update({'12':12})
# count = 13
# for num in range(7, 12):
#     time_dict2.update({str(num): count})
#     count+=1


def test_func():
    # result = (time_dict[close_hour_Var.get()+'pm'] - time_dict[hour_optionvar.get()+'am']) 
...
user_current_time_option_menu = tk.OptionMenu(user_current_entry_frame, hour_optionvar, *time_dict2.keys())
user_current_time_option_menu.pack(side = 'left')

minute_optionvar = tk.StringVar()
minute_optionvar.set("00")
samp_option_menu = tk.OptionMenu(user_current_entry_frame, minute_optionvar, *range(5, 60, 5))
...
close_minute_option_menu = tk.OptionMenu(user_close_hour_option_frame, closeminute_optionvar, *range(5, 60, 5)) #creates option menu
